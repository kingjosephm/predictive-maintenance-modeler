...
from utils import set_seeds  # needed for reproducibility if using SMOTE
set_seeds(42)
...
class DataProcessor:
    ...
    def __init__(self, 
        ...
        self.seed = seed
        self.model = model
        self.lag_length = 0 if sampling_n == 1 else lag_length  # lags not possible with sampling_n=1
        self.sampling_n = max(1, sampling_n)  # minimum 1
    ...
    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Initial preprocessing of data including: 
            ...
            4) creation of final outcome features for scikit-survival model, based the last timepoint per `self.unit_identifier`
        ...
        # Note: scikit-survival expect this as bool - will need to modify if use other 3rd party module
        assert df[self.target_feature].nunique() == 2, f"Error! Maximum number of allowed categories in target feature is 2! However {df[self.target_feature].nunique()} were found!"
        assert df[self.target_feature].min() == 0, "Error! Expected target feature to be coded {0, 1}"
        assert df[self.target_feature].max() == 1, "Error! Expected target feature to be coded {0, 1}"
        df[self.target_feature] = np.where(df[self.target_feature] == 1, True, False)
        
        last_cycle = df[df[self.time_identifier] == df.groupby(self.unit_identifier)[self.time_identifier].transform('max')][
            [self.unit_identifier, self.target_feature, self.time_identifier]].set_index(self.unit_identifier)
        ...
        if self.sampling_n > 1:
            ...
            logging.info(f"Keeping only last observation per '{self.unit_identifier}'..")
        ...
        # Add feature lags [optional], else renames columns to `{colname}_l0`
        if self.lag_length > 0:
            df = self.feature_lags(df)
        else:
            df = df.drop(columns=[self.target_feature])  # drop target feature, since it's already in `last_cycle`
            cols = [i for i in df.columns if "Cat" not in i]  # note - "Cat" is dataset specific, remove if not needed
            df = df.rename(columns={col: f"{col}_l0" for col in cols})  # rename contemporary features to `_l0` for consistency, note excludes invariant "Cat" cols

        # Concat `self.target_feature` so that this column is the same for all `self.time_identifier` (time periods) for a given `self.unit_identifier`
        df = df.merge(last_cycle, left_index=True, right_index=True)
    ...
    def feature_lags(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        assert self.lag_length > 0, "Error! `self.lag_length` must be greater than 0!"
        
        wide_format = pd.DataFrame()
        
        # minimum number of observations per unit
        # note - adding lags > min_group_obs will result in pointless features (all NaNs, which are filled with bfill)
        min_group_obs = df.groupby(self.unit_identifier)[self.time_identifier].count().min()
        ...
        for col in cols:
            concat_list = []
            for i in reversed(range(1, min(min_group_obs, self.lag_length+1))):
    ...
    def sample_group_observations(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        quantiles (bins) is determined by min(`self.sampling_n`, total unit observations).
        ...
        """
        quantile = df.groupby([self.unit_identifier])[self.time_identifier].apply(lambda x:  pd.qcut(x, q=min(self.sampling_n, len(x)), labels=False)).reset_index(drop=True)
